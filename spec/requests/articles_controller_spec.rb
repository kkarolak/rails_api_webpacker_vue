  ...
  context 'Methods' do
    ...
    describe 'GET articles#show' do
      it "show action should be success" do
        get '/articles', params: {id: article.id}
        expect(response).to have_http_status(:success)
      end
    end
    describe 'POST articles#create' do
      it 'create new article if authenticated user is admin' do
        expect{(post '/articles/', params: {article: params, format: :json}, headers: {'Authorization': "Barer #{jwt_token.result}"})}.to change(Article, :count).by(1)
      end
      it 'do not create new article if authenticated user is not admin' do
        ...
        expect{(post '/articles/', params: {article: params, format: :json}, headers: {'Authorization': "Barer #{token.result}"})}.to change(Article, :count).by(0)
      end

    end
    describe 'PUT # update' do
      it 'update article info' do
        put "/articles/#{article.id}", params:{id: article.id, article: params}, headers: {'Authorization': "Barer #{jwt_token.result}"}
        article.reload
      ...
      it 'shouldnt update article info' do
        token = AuthenticateUser.new(user1.email, user1.password).call
        put "/articles/#{article.id}", params:{article: params}, headers: {'Authorization': "Barer #{token.result}"}
        article.reload
    ...
    describe 'DELETE # destroy' do
      it 'should delete article' do
        ...
        expect { delete "/articles/#{article.id}", params: { id: article.id }, headers:{'Authorization': "Barer #{jwt_token.result}"} }.to change(Article, :count).by(-1)
      end
      it 'shouldnt delete article' do
        token = AuthenticateUser.new(user1.email, user1.password).call
        article = Article.create!(id:6 ,title:"First",description:"First Article",user_id: user.id)
        expect { delete "/articles/#{article.id}", params: { id: article.id }, headers:{'Authorization': "Barer #{token.result}"} }.to change(Article, :count).by(0)
      end
    end
    end
  end
