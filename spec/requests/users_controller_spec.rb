  ...
  context 'Methods' do
    ...
    describe 'POST users#register' do
      it 'register new user' do
        expect{(post '/auth/register', params: {name:"Test", email:"test@o2.pl", password:"password"})}.to change(User, :count).by(1)
      end
    end
    describe 'PUT # update' do
      it 'update user info' do
        put "/users/#{user.id}", params:{name: "test", email: "test@netkodo.com"}, headers: {'Authorization': "Barer #{jwt_token.result}"}
        user.reload
        #puts response.body
      ...
      it 'shouldnt update user info' do
        ...
        put "/users/#{user1.id}", params:{name: "test", email: "test@netkodo.com"}, headers: {'Authorization': "Barer #{token.result}"}
        user.reload
    ...
    describe 'DELETE # destroy' do
      it 'should delete user' do
        ...
        expect { delete "/users/#{user2.id}", params: { id: user2.id }, headers:{'Authorization': "Barer #{jwt_token.result}"} }.to change(User, :count).by(-1)
      end
      it 'shouldnt delete article' do
        token = AuthenticateUser.new(user1.email, user1.password).call
        expect { delete "/users/#{user.id}", params: { id: user.id }, headers:{'Authorization': "Barer #{token.result}"} }.to change(Article, :count).by(0)
      end
    end
    describe '#login' do
      it 'should receive token' do
      post '/auth/login', params: {password:user.password, email:user.email}
